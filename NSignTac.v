...
intros n m p H1 H2; apply Nlt_lt; repeat rewrite Nmult_mult.
apply mult_lt_compat_l; apply lt_Nlt; repeat rewrite nat_N_correct; auto.
Qed.

Theorem Nmult_le_compat_l: forall n m p, n <= m -> p * n <= p * m.
intros n m p H1; apply Nle_le; repeat rewrite Nmult_mult.
apply Mult.mult_le_compat_l; apply le_Nle; repeat rewrite nat_N_correct; auto.
Qed.

Theorem Nmult_ge_compat_l: forall n m p, n >= m -> p * n >= p * m.
intros n m p H1; apply Nge_ge; repeat rewrite Nmult_mult.
apply mult_ge_compat_l; apply ge_Nge; repeat rewrite nat_N_correct; auto.
Qed.

Theorem Nmult_gt_compat_l: forall n m p, n > m -> p > 0 -> p * n > p * m.
intros n m p H1 H2; apply Ngt_gt; repeat rewrite Nmult_mult.
apply mult_gt_compat_l; apply gt_Ngt; repeat rewrite nat_N_correct; auto.
Qed.
...
repeat rewrite <- Nmult_mult; apply lt_Nlt; repeat rewrite nat_N_correct; auto.
Qed.
...
repeat rewrite <- Nmult_mult; apply gt_Ngt; repeat rewrite nat_N_correct; auto.
Qed.
...
repeat rewrite <- Nmult_mult; apply le_Nle; repeat rewrite nat_N_correct; auto.
apply lt_Nlt; repeat rewrite nat_N_correct; auto.
Qed.
...
repeat rewrite <- Nmult_mult; apply ge_Nge; repeat rewrite nat_N_correct; auto.
apply lt_Nlt; repeat rewrite nat_N_correct; auto.
Qed.

Theorem Nlt_mult_0: forall a b, 0 < a -> 0 < b -> 0 < a * b.
intros a b H1 H2; apply Nlt_lt; rewrite Nmult_mult; apply lt_mult_0; apply lt_Nlt; repeat rewrite nat_N_correct; auto.
Qed.

Theorem Ngt_mult_0: forall a b, a > 0 -> b > 0 -> a * b > 0.
intros a b H1 H2; apply Ngt_gt; rewrite Nmult_mult; apply gt_mult_0; apply gt_Ngt; repeat rewrite nat_N_correct; auto.
Qed.

Theorem Nlt_mult_rev_0_l: forall a b, 0 < a * b ->  0 < a .
intros a b H1; apply Nlt_lt; apply lt_mult_rev_0_l with (nat_of_N b). 
rewrite <- Nmult_mult; apply lt_Nlt; repeat rewrite nat_N_correct; auto.
Qed.
...
rewrite <- Nmult_mult; apply gt_Ngt; repeat rewrite nat_N_correct; auto.
Qed.
...
intros n H1; rewrite <- (nat_N_correct n).
rewrite (le_0_eq_0 (nat_of_N n)); auto.
apply le_Nle; rewrite nat_N_correct; auto.
Qed.
...
apply ge_Nge; rewrite nat_N_correct; auto.
Qed.
